...
export default function RevalidatePage() {
  const [status, setStatus] = useState<Record<string, RevalidateStatus>>({
    ...
    collections: { type: 'collections', loading: false, message: '', success: null },
    categories: { type: 'categories', loading: false, message: '', success: null },
    all: { type: 'all', loading: false, message: '', success: null },
  })

  const handleRevalidate = async (type: 'products' | 'collections' | 'categories' | 'all') => {
    setStatus((prev) => ({
      ...prev,
    ...
    <div className="min-h-screen bg-[#0a0a0a]">
      ...
      <div className="border-b border-[#2d2d2d] bg-[#1a1a1a]">
        <div className="max-w-6xl mx-auto px-6 py-8">
          ...
          <p className="text-[#b0b0b0]">
            Manually trigger cache revalidation for your content without deploying
          </p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="bg-[#1a1a1a] border border-[#2d2d2d] rounded-sm p-6 mb-8">
          <h2 className="text-lg font-semibold text-[#fafafa] mb-3">How It Works</h2>
          <ul className="space-y-2 text-sm text-[#b0b0b0]">
            <li className="flex items-start gap-3">
              <span className="text-[#d4af37] mt-0.5">→</span>
              <span>
                <strong>Manual Revalidation:</strong> Click any button below to immediately refresh content cache
              </span>
            </li>
            <li className="flex items-start gap-3">
              <span className="text-[#d4af37] mt-0.5">→</span>
              <span>
                <strong>Automatic Webhooks:</strong> Set up Sanity webhooks to trigger revalidation automatically when content changes
              </span>
            </li>
            <li className="flex items-start gap-3">
              <span className="text-[#d4af37] mt-0.5">→</span>
              <span>
                <strong>Zero Downtime:</strong> Content updates are applied instantly without redeploying your application
              </span>
            </li>
          </ul>
        </div>
        ...
        <div className="bg-[#1a1a1a] border border-[#2d2d2d] rounded-sm p-6">
          ...
          <div className="space-y-4 text-sm text-[#b0b0b0]">
            <p>
              To automatically revalidate content when you publish changes in Sanity Studio:
            </p>
            <ol className="space-y-3 ml-4 list-decimal">
              ...
              <li>
                Set the URL to: <code className="bg-[#0a0a0a] px-2 py-1 rounded text-[#d4af37]">{typeof window !== 'undefined' ? `${window.location.origin}/api/revalidate` : 'YOUR_DOMAIN/api/revalidate'}</code>
              </li>
              <li>
                Add header: <code className="bg-[#0a0a0a] px-2 py-1 rounded text-[#d4af37]">X-Sanity-Webhook-Secret: YOUR_SECRET</code>
              </li>
              <li>Select events: Publish, Create, Update, Delete</li>
              <li>Click Create</li>
            </ol>
            <p className="mt-4 text-[#b0b0b0]">
              Set your webhook secret in the environment variable: <code className="bg-[#0a0a0a] px-2 py-1 rounded text-[#d4af37]">SANITY_WEBHOOK_SECRET</code>
            </p>
          </div>
        </div>
      </div>
